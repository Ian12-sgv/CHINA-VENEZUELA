...
{
    public async Task<CompraRecibida?> ObtenerPorIdAsync(Guid id, CancellationToken cancellationToken) => await context.ComprasRecibidas.SingleOrDefaultAsync(x => x.Id == id, cancellationToken);
    public async Task<IReadOnlyList<CompraRecibida>> ObtenerTodasAsync(CancellationToken cancellationToken) => await context.ComprasRecibidas.AsNoTracking().OrderByDescending(x => x.FechaLlegada ?? x.FechaSalida).ToListAsync(cancellationToken);
