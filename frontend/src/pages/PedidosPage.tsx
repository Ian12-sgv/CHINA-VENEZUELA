...
import { catalogosApi, fichasTecnicasApi, pedidosApi, receptoresApi } from '../api'
...
import type { CrearProductoPedidoRequest, FichaTecnicaApi, PaginaProductosPedido, PedidoResumen, ProductoPedido, TipoImagenProductoPedido } from '../types'
...
export function PedidosPage() {
  ...
  const [fichaPorReferencia, setFichaPorReferencia] = useState<FichaTecnicaApi | null>(null)
  ...
  return <section className="page-grid">
    ...
    <article className="card form-card">
      ...
      <form className="form-grid" onSubmit={(e: FormEvent) => { e.preventDefault(); guardar.mutate() }}>
        ...
        <label><span>Referencia asignada</span><input required value={form.referenciaAsignada} onChange={(e) => { field('referenciaAsignada', e.target.value); setFichaPorReferencia(null) }} onKeyDown={(e) => { if (e.key !== 'Enter') return; e.preventDefault(); const referencia = form.referenciaAsignada.trim(); if (!referencia) return; void fichasTecnicasApi.porReferencia(referencia).then(setFichaPorReferencia).catch(() => setFichaPorReferencia(null)) }} /></label>
{fichaPorReferencia && <label className="ficha-referencia-pedido"><span>Ficha técnica vinculada</span><input readOnly value={`${fichaPorReferencia.codigo}  ${fichaPorReferencia.categoria} · ${fichaPorReferencia.linea}`} aria-label="Ficha técnica vinculada" /></label>}        <label><span>Código barra asignado</span><input required value={form.codigoBarraAsignado} onChange={(e) => field('codigoBarraAsignado', e.target.value)} /></label>
...
function Tabla({ pagina, grupos, grupoFiltro, busqueda, filtroEnvio, cargar, exportando, buscar, filtrarGrupo, filtrarEnvio, irPagina, editar, eliminar, duplicar, enviar, exportar }: { pagina: PaginaProductosPedido | undefined; grupos: PedidoResumen[]; grupoFiltro: string; busqueda: string; filtroEnvio: FiltroEnvio; cargar: boolean; exportando: 'excel' | 'pdf' | null; buscar: (x: string) => void; filtrarGrupo: (x: string) => void; filtrarEnvio: (x: FiltroEnvio) => void; irPagina: (x: number) => void; editar: (x: ProductoPedido) => void; eliminar: (x: ProductoPedido) => void; duplicar: (x: ProductoPedido) => void; enviar: (x: ProductoPedido) => void; exportar: (tipo: 'excel' | 'pdf', producto?: ProductoPedido, pedido?: PedidoResumen, seleccionados?: ProductoPedido[]) => void }) {
  ...
  const totalColumnasCatalogo = 22 + marcasBultoColumnas.length
