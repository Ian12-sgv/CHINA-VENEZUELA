...
export function UsuariosPage() {
  ...
  return <section className="page-grid"><div className="page-heading"><div><span className="eyebrow">Seguridad</span><h2>Usuarios</h2><p>Crea, edita, elimina usuarios y asigna sus grupos.</p></div></div>
    <article className="card catalog-card"><div className="form-title"><div><h3>{editando ? 'Editar usuario' : 'Nuevo usuario'}</h3><p>{editando ? 'Deja la contraseÃ±a vacÃ­a para conservar la actual.' : 'Selecciona al menos un grupo para el usuario.'}</p></div>{editando && <button className="link-button" onClick={cancelar}>Cancelar</button>}</div><form className="form-grid" onSubmit={submit}><label><span>Nombre</span><input value={form.nombre} onChange={event => setForm({ ...form, nombre: event.target.value })} required maxLength={200} /></label><label><span>Correo</span><input type="email" value={form.correo} onChange={event => setForm({ ...form, correo: event.target.value })} required maxLength={254} placeholder="correo@ejemplo.com" /></label><label><span>Codigo de usuario</span><input value={form.codigoUsuario} onChange={event => setForm({ ...form, codigoUsuario: event.target.value })} required disabled={editando !== null} maxLength={50} /></label><label><span>ContraseÃ±a</span><input type="password" value={form.contrasena} onChange={event => setForm({ ...form, contrasena: event.target.value })} required={!editando} minLength={editando ? undefined : 8} placeholder={editando ? 'Sin cambios' : 'Minimo 8 caracteres'} /></label><label className="user-status"><span>Estado</span><input type="checkbox" checked={form.status} onChange={event => setForm({ ...form, status: event.target.checked })} /> Activo</label><div className="description-field"><span className="groups-label">Grupos</span><div className="group-checkboxes">{grupos.data?.map(grupo => <label key={grupo.nombre} className="group-checkbox"><input type="checkbox" checked={form.grupos.includes(grupo.nombre)} onChange={event => alternarGrupo(grupo.nombre, event.target.checked)} /><span>{grupo.nombre}</span></label>)}</div></div><div className="description-field">{error && <p className="error">{error}</p>}{mensaje && <p className="success">{mensaje}</p>}<button className="primary" disabled={guardar.isPending || form.grupos.length === 0}>{guardar.isPending ? 'Guardando...' : editando ? 'Guardar cambios' : 'Crear usuario'}</button></div></form></article>
